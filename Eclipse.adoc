...
You may instead follow https://github.com/oliviercailloux/java-course/blob/master/Dev%20tools/Eclipse.adoc#configuration[these] instructions, which will add a “GoogleStyle + block + braced” formatter style configured exactly as when following Method 1 (TODO not updated to last version yet), and also tweak multiple other aspects of Eclipse.
...
== Sort and compare your settings
...
You obtain an `Untitled.xml` file with sorted settings elements, which you can then compare using any diff program to other similarly sorted Eclipse formatter configuration files. (It seems to me that Eclipse’s export ordering is not deterministic, hence the required sorting step.)
