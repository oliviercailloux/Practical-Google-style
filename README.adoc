...
* Allowed one-letter abbreviations: one letter words such as `X` (the https://en.wikipedia.org/wiki/X_Window_System[X Window System] or the https://en.wikipedia.org/wiki/X_%28social_network%29[social network]) or `C` (the https://en.wikipedia.org/wiki/C_(programming_language)[programming language]), lead to the perfectly valid (per GJSG) names `XKey` or `CCompiler`. The Checkstyle default configuration forbids this. It can be changed https://github.com/checkstyle/checkstyle/issues/14239[either] by explicitly listing every one-letter word of the English language, or by tolerating all one-letter abbreviations (equivalently, considering that all one-letter words exist). I chose the latter option.
** Incidentally, this also permits the name `TOptional` with a `T` prefix for a “Throwable” generalization of the class (which arguably does not comply with the GJSG but I find this a useful extension).
